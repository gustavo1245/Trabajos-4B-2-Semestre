...
class Usuario:
    ...
    def guardar_usuario(cls, data):
        ...
        query = "INSERT INTO usuarios (nombre, apellido, fecha_nacimiento, edad, email, password) VALUES (%(nombre)s, %(apellido)s, %(fecha_nacimiento)s, %(edad)s, %(email)s, %(password)s);"
    ...
    def validar_registro(usuario):
        ...
        try:
            fecha_nacimiento = datetime.strptime(usuario['fecha_nacimiento'], '%Y-%m-%d').date()
            today = date.today()
            edad = today.year - fecha_nacimiento.year - ((today.month, today.day) < (fecha_nacimiento.month, fecha_nacimiento.day))
            
            if edad < 18:
                flash("Debes ser mayor de 18 años para registrarte.", 'registro')
                is_valid = False
            elif edad > 120:
                flash("Por favor, ingresa una fecha de nacimiento válida.", 'registro')
                is_valid = False
        except ValueError:
            flash("Formato de fecha de nacimiento inválido.", 'registro')
            is_valid = False
        
        if not usuario['edad'].isdigit() or int(usuario['edad']) < 18:
            flash("Debes ser mayor de 18 años para registrarte.", 'registro')
            is_valid = False
        if int(usuario['edad']) > 120:
            flash("Por favor, ingresa una edad válida.", 'registro')
            is_valid = False
